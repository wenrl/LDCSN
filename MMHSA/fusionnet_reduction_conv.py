...
from Trans_cnn.conformer import C2M,M2C,Block_unit_conv
...
class feauture_Reduction(nn.Module):
    def __init__(self, inplane, outplane):
        ...
        self.act_layer1 = nn.PReLU(outplane)
...
class UpperFaceAttention(nn.Module):
    def __init__(self, C2, C3, C4, block):
        ...
        self.ms1 = Block_unit_conv(dim=C4, num_heads=4, mlp_ratio=3, qkv_bias=False, qk_scale=None,
                              drop=.1, attn_drop=.1, drop_path=.0)
        self.ms2 = Block_unit_conv(dim=C4, num_heads=4, mlp_ratio=3, qkv_bias=False, qk_scale=None,
                              drop=.1, attn_drop=.1, drop_path=.0)
        self.ms3 = Block_unit_conv(dim=C4, num_heads=4, mlp_ratio=3, qkv_bias=False, qk_scale=None,
                              drop=.1, attn_drop=.1, drop_path=.0)
    ...
    def forward(self, inputs):
        ...
        x1 = self.ms1(x1).reshape(B,C,H,W)
        x2 = self.ms2(x2).reshape(B,C,H,W)
        x3 = self.ms3(x3).reshape(B,C,H,W)
...
class BlockIR(nn.Module):
    def __init__(self, inplanes, planes, stride, dim_match, use_att=False,
                 dw_stride=2, embed_dim=256, num_heads=4, mlp_ratio=4.,
                 qkv_bias=True, qk_scale=None, drop_rate=0.1, attn_drop_rate=0.1, drop_path_rate=0,
                 last_fusion=False, num_med_block=0, groups=1,patch_size = 4, fcu=False):
        ...
        self.squeeze_block = C2M(inplanes=inplanes, outplanes=embed_dim, dw_stride=1)
        self.expand_block = M2C(inplanes=embed_dim, outplanes=planes, stride=stride)
        self.trans_block = Block_unit_conv(
